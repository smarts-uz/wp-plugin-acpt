...
class BarcodeFieldValuesTest extends AbstractTestCase
{
	/**
	 * @test
	 */
	public function can_add_acpt_meta_field_value()
	{
				...
				[
					...
					'fields' => [
						[
							'name' => 'barcode_field',
							'label' => 'barcode field',
							'type' => MetaFieldModel::BARCODE_TYPE,
							'showInArchive' => false,
							'isRequired' => false,
							'defaultValue' => "foo",
		...
		foreach ($this->dataProvider() as $key => $value){
			...
			$this->assertEquals($acpt_field['text'], "this is a test");
			$this->assertEquals($acpt_field['value']['format'], "code128");
			$this->assertEquals($acpt_field['value']['color'], "#333333");
			$this->stringContains($acpt_field['value']['svg'], "svg width");
		}
	}
	...
	public function can_edit_acpt_meta_field_value()
	{
		foreach ($this->dataProvider() as $key => $value){
            ...
            $this->assertEquals($acpt_field['text'], "12345670");
            $this->assertEquals($acpt_field['value']['format'], "ean8");
            $this->assertEquals($acpt_field['value']['color'], "#666666");
            $this->stringContains($acpt_field['value']['svg'], "svg width");
		}
	}
