...
class ACPT_Admin
{
    ...
    private function setStaticCssAssets()
    {
	    ...
	    if(in_array($pagenow, $allowedPages) and !$this->isACPTAppPage()){
		    $this->staticCssAssets = [
			    ...
			    'admin_css' => plugins_url(  ACPT_DEV_MODE ? 'advanced-custom-post-type/assets/static/css/admin.css' : 'advanced-custom-post-type/assets/static/css/admin.min.css'),
		    ];
	    }
    }
    ...
    private function setStaticJsAssets()
    {
	    ...
	    if(in_array($pagenow, $allowedPages) and !$this->isACPTAppPage()){
		    ...
		    $jsAssets['admin_js'] = [
			    'path' => plugins_url( ACPT_DEV_MODE ? 'advanced-custom-post-type/assets/static/js/admin.js' : 'advanced-custom-post-type/assets/static/js/admin.min.js'),
			    'dep'  => ['jquery'],
		    ];
	    }
	    ...
	    if($pagenow === "edit-comments.php"){
		    $jsAssets['comment_quick_edit_js'] = [
			    'path' => plugins_url( ACPT_DEV_MODE ? 'advanced-custom-post-type/assets/static/js/comment-quick-edit.js' : 'advanced-custom-post-type/assets/static/js/comment-quick-edit.min.js'),
			    'dep'  => ['jquery'],
		    ];
	    }
                            ...
                            foreach ($actions as $action){
                                add_action( 'admin_init', function () use($action, $postTypeName){
                                    add_action( $action.'_'.$postTypeName, function ($post_id, $post) use($action, $postTypeName){
                                        Posts::invalidPostsQueryCache($postTypeName);
                                    }, 10, 2 );
                                });
                            }
                        }
