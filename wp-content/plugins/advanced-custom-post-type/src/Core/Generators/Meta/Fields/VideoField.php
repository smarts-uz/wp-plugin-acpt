...
class VideoField extends AbstractField
{
	public function render()
	{
		...
		if($this->isChild() or $this->isNestedInABlock()){
			...
			$field .= '<input readonly '.$this->required().' id="'.$id.'['.$this->getIndex().']" name="'. esc_attr($this->getIdName()).'[value]" type="text" class="hidden" value="' .esc_attr($this->getDefaultValue()) .'" '. $this->appendDataValidateAndLogicAttributes() .'>';
		} else {
