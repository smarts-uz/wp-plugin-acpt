...
class IconField extends AbstractField
{
	public function render()
	{
		...
		if($this->isChild() or $this->isNestedInABlock()){
			$id = "icon_".Strings::generateRandomId();
			...
			$field .= '<input type="hidden" class="acpt-icon-picker-value" data-target-id="'. $id.'" id="'.$this->getIdName().'" name="'. esc_attr($this->getIdName()).'[value]" value="'.Strings::htmlspecialchars($this->getDefaultValue()).'"/>';
			$field .= $this->renderIconPicker($id);
		} else {
			$field = '<input type="hidden" name="'. esc_attr($this->getIdName()).'_type" value="'.MetaFieldModel::ICON_TYPE.'">';
			$field .= '<input type="hidden" class="acpt-icon-picker-value" data-target-id="'. $this->getIdName().'" id="'.$this->getIdName().'" name="'. esc_attr($this->getIdName()).'" value="'.Strings::htmlspecialchars($this->getDefaultValue()).'"/>';
	...
	protected function renderIconPicker($id)
	{
		...
		if(!empty($this->getDefaultValue()) and is_string($this->getDefaultValue())){
		    $svg = $this->getDefaultValue();
			$extractColorAndSize = $this->extractColorAndSize($svg);
