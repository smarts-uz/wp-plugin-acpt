...
class ImportRepository extends AbstractRepository
{
		...
		foreach ($groups as $group){
			...
			if(isset($group['boxes'])) {
				foreach ( $group['boxes'] as $box ) {
                    ...
                    if(isset($box['visibilityConditions'])){
                        foreach ( $box['visibilityConditions'] as $visibilityCondition ) {
                            $sql = "
	...
	private static function importMetaField($boxId, $field, $parentFieldId = null, $parentBlockId = null)
	{
		...
		if(isset($field['permissions']) and is_array($field['permissions'])){
			foreach ($field['permissions'] as $index => $permission){
				$sql = "
			            INSERT INTO `".ACPT_DB::prefixedTableName(ACPT_DB::TABLE_PERMISSION)."` 
			            (`id`,
			            `entity_id` ,
