        ...
        switch ($field['type']){
            ...
            case MetaFieldModel::AUDIO_TYPE:
                ...
                if(is_string($rawData)){
                    $rawData = WPAttachment::fromUrl($rawData);
                }
                ...
                return Audio::single($rawData, $customPlayer, $style);
            ...
            case MetaFieldModel::AUDIO_MULTI_TYPE:
                ...
                // for fields nested in a repeater
                foreach ($rawData as $index => $attachment){
                    if(is_string($attachment)){
                        $rawData[$index] = WPAttachment::fromUrl($attachment);
                    }
                }

                return Audio::playlist($rawData, $customPlayer, $style);
            ...
            case MetaFieldModel::PHONE_TYPE:
                ...
                $phone = Phone::format($rawData, null, $format);

                if(isset($attributes['display']) and $attributes['display'] === 'link'){
                    return '<a href="'.Phone::format($phone, null, Phone::FORMAT_RFC3966).'" target="_blank">'.$phone.'</a>';
                }
