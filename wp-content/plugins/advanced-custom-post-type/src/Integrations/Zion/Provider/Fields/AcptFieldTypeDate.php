	...
	public function render($fieldObject)
	{
		...
		switch ($metaFieldModel->getType()){
			case MetaFieldModel::DATE_TYPE:
				echo ($format !== null) ? date_i18n($format, strtotime($rawValue)) : $rawValue;
				break;
			...
			case MetaFieldModel::DATE_RANGE_TYPE:
				...
				$startDate = ($format !== null) ? date_i18n($format, strtotime($start)) : $start;
				$toDate = ($format !== null) ? date_i18n($format, strtotime($end)) : $end;
