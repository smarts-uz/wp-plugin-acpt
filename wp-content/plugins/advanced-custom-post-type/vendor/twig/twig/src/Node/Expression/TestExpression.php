...
class TestExpression extends CallExpression implements ReturnBoolInterface
{
    ...
    public function __construct(Node $node, string|TwigTest $test, ?Node $arguments, int $lineno)
    {
        ...
        if ($test instanceof TwigTest) {
            $this->setAttribute('acpt_twig_callable', $test);
        }
    ...
    public function compile(Compiler $compiler): void
    {
        ...
        if ($this->hasAttribute('acpt_twig_callable')) {
            $name = $this->getAttribute('acpt_twig_callable')->getName();
            if ($name !== $this->getAttribute('name')) {
                ...
                $this->removeAttribute('acpt_twig_callable');
            }
        }

        if (!$this->hasAttribute('acpt_twig_callable')) {
            $this->setAttribute('acpt_twig_callable', $compiler->getEnvironment()->getTest($this->getAttribute('name')));
        }
