...
class Google_Logger_File extends Google_Logger_Abstract
{
    ...
    public function __construct(Google_Client $client)
    {
        ...
        $file = $client->getClassConfig('VendorDuplicator\\Google_Logger_File', 'file');
        if (!\is_string($file) && !\is_resource($file)) {
            throw new Google_Logger_Exception('File logger requires a filename or a valid file pointer');
        }
        $mode = $client->getClassConfig('VendorDuplicator\\Google_Logger_File', 'mode');
        if (!$mode) {
            $this->mode = $mode;
        }
        $this->lock = (bool) $client->getClassConfig('VendorDuplicator\\Google_Logger_File', 'lock');
        $this->file = $file;
    }
    /**
     * {@inheritdoc}
 ...
 * @internal
 */
\class_alias('VendorDuplicator\\Google_Logger_File', 'VendorDuplicator\\Google_Logger_File', \false);
