 ...
 */
if (!\class_exists('VendorDuplicator\\Google_Client')) {
...
class Google_Auth_OAuth2 extends Google_Auth_Abstract
{
    ...
    public function authenticate($code, $crossClient = \false)
    {
        ...
        $arguments = array('code' => $code, 'grant_type' => 'authorization_code', 'client_id' => $this->client->getClassConfig($this, 'client_id'), 'client_secret' => $this->client->getClassConfig($this, 'client_secret'));
        if ($crossClient !== \true) {
            $arguments['redirect_uri'] = $this->client->getClassConfig($this, 'redirect_uri');
        }
        // We got here from the redirect from a successful authorization grant,
        // fetch the access token
        $request = new Google_Http_Request(self::OAUTH2_TOKEN_URI, 'POST', array(), $arguments);
        $request->disableGzip();
        $response = $this->client->getIo()->makeRequest($request);
        if ($response->getResponseHttpCode() == 200) {
            $this->setAccessToken($response->getResponseBody());
            $this->token['created'] = \time();
            return $this->getAccessToken();
        } else {
            $decodedResponse = \json_decode($response->getResponseBody(), \true);
    ...
    public function isAccessTokenExpired()
    {
        ...
        $expired = $this->token['created'] + ($this->token['expires_in'] - 30) < \time();
        return $expired;
    }
    ...
    public function retrieveCertsFromLocation($url)
    {
        ...
        if ($request->getResponseHttpCode() == 200) {
            $certs = \json_decode($request->getResponseBody(), \true);
            if ($certs) {
                return $certs;
            }
        }
    ...
    public function verifyIdToken($id_token = null, $audience = null)
    {
        ...
        return $this->verifySignedJwtWithCerts($id_token, $certs, $audience, array(self::OAUTH2_ISSUER, self::OAUTH2_ISSUER_HTTPS));
    }
    /**
    ...
    public function verifySignedJwtWithCerts($jwt, $certs, $required_audience, $issuer = null, $max_expiry = null)
    {
        ...
        // Parse envelope.
        $envelope = \json_decode(Google_Utils::urlSafeB64Decode($segments[0]), \true);
        if (!$envelope) {
            throw new Google_Auth_Exception("Can't parse token envelope: " . $segments[0]);
        }
        // Parse token
        $json_body = Google_Utils::urlSafeB64Decode($segments[1]);
        $payload = \json_decode($json_body, \true);
        if (!$payload) {
        ...
        if ($issuer && !\in_array($iss, (array) $issuer)) {
            throw new Google_Auth_Exception(\sprintf("Invalid issuer, %s not in %s: %s", $iss, "[" . \implode(",", (array) $issuer) . "]", $json_body));
        }
        // Check audience
        $aud = $payload["aud"];
 ...
 * @internal
 */
\class_alias('VendorDuplicator\\Google_Auth_OAuth2', 'VendorDuplicator\\Google_Auth_OAuth2', \false);
