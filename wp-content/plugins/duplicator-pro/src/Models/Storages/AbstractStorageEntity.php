...
abstract class AbstractStorageEntity extends DUP_PRO_Storage_Entity implements AbstractStorageEntityInterface, ModelMigrateSettingsInterface
{
    ...
    public function purgeOldPackages()
    {
        ...
        try {
            ...
            usort($packagesList, function ($a, $b) use ($adapter) {
                if (($aInfo = $adapter->getPathInfo($a)) === false) {
                    return 0;
                }
                if (($bInfo = $adapter->getPathInfo($b)) === false) {
                    return 0;
                }
                if ($aInfo->created == $bInfo->created) {
                    return 0;
                } elseif ($aInfo->modified < $bInfo->modified) {
                    return -1;
                } else {
                    return 1;
                }
